...
export class MovieService {
  ...
  getFavourites():any{
    let search_url = `http://localhost:8080/movie/movieservice`;
    return this.http.get(search_url);
  }
  addFavourites(newFav: favourites):any{
    let post_url = `http://localhost:8080/movie/movieservice`;
    let b= this.http.post(post_url,newFav,httpOptions);
    return b;
  }
  checkIfFav(id):any{
    let _url = `http://localhost:8080/movie/movieservice/id/${id}`;
    return this.http.get(_url);
  }
  removeFromFavourites(id): any{
    let _url = `http://localhost:8080/movie/movieservice/${id}`;
    return this.http.delete(_url,httpOptions);
  }
  updateComments(favObj):any{
    let _url = `http://localhost:8080/movie/movieservice`;
